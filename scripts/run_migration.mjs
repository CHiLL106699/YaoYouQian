...
try {
  ...
  console.log('✅ All tables, indexes, and RLS created successfully!');
  
  // Verify
  const res = await client.query(`
    SELECT table_name FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name IN ('medical_records','medical_photos','consent_forms','appointment_reminders','appointment_locks')
    ORDER BY table_name;
  `);
