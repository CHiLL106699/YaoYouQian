...
/**
 * YoCHiLLSAAS - Complete Database Schema
 * 多租戶預約管理系統完整資料表定義
 */
...
export const services = mysqlTable("services", {
  ...
  duration: int("duration").notNull(), // 分鐘
...
export const bookingSlotLimits = mysqlTable("booking_slot_limits", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenant_id").notNull(),
  date: timestamp("date").notNull(),
  timeSlot: varchar("time_slot", { length: 10 }).notNull(),
  maxCapacity: int("max_capacity").default(5).notNull(),
...
export const timeSlotTemplates = mysqlTable("time_slot_templates", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenant_id").notNull(),
  name: varchar("name", { length: 100 }).notNull(),
  description: text("description"),
...
export const doseCalculations = mysqlTable("dose_calculations", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenant_id").notNull(),
  customerId: int("customer_id").notNull(),
...
export const rescheduleApprovals = mysqlTable("reschedule_approvals", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenant_id").notNull(),
  appointmentId: int("appointment_id").notNull(),
...
export const slotLimits = mysqlTable("slot_limits", {
  id: int("id").autoincrement().primaryKey(),
  tenantId: int("tenant_id").notNull(),
